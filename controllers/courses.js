...
exports.updateCourse = asyncHandler(async (req, res, next) => {
    ...
    if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorResponse(`User ${req.user.id} is not authorized to update course to course ${course._id}`), 401)
    }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
    ...
    //make sure that user is course owner
    if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorResponse(`User ${req.user.id} is not authorized to remove course to course ${course._id}`), 401)
    }
